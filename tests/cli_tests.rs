...
fn run_command(args: &[&str]) -> (String, String) {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_numby"))
        .args(&full_args)
        .output()
...
fn test_word_numbers_basic() {
    let (stdout, _) = run_command(&["ten plus five"]);
...
fn test_word_numbers_mixed_case() {
    let (stdout, _) = run_command(&["Twenty MINUS four"]);
...
fn test_yesterday_plus_days() {
    let (stdout, _) = run_command(&["yesterday + 10 days"]);
...
fn test_tomorrow_minus_days() {
    let (stdout, _) = run_command(&["tomorrow - 1 day"]);
    let re = Regex::new(r"\d{4}-\d{2}-\d{2}").unwrap();
    assert!(re.is_match(&stdout), "expected date output, got {}", stdout);
}
...
fn test_yesterday_plus_days_chain() {
    let (stdout, _) = run_command(&["yesterday + 1 day + 2 days"]);
    let re = Regex::new(r"\d{4}-\d{2}-\d{2}").unwrap();
    assert!(re.is_match(&stdout), "expected date output, got {}", stdout);
}
...
fn test_currency_conversion() {
    ...
    let (stdout, _) = run_command(&["--no-update", "--rate", "EUR:0.85", "10 usd in eur"]);
...
fn test_currency_symbol_dollar_to_eur() {
    ...
    let (stdout, _) = run_command(&["--no-update", "--rate", "EUR:0.85", "100$ to eur"]);
    assert!(stdout.contains("eur"));
    // 100 * 0.85 = 85.00 EUR
...
fn test_currency_symbol_euro_to_usd() {
    ...
    let (stdout, _) = run_command(&["--no-update", "--rate", "EUR:0.85", "€50 to usd"]);
...
fn test_byn_conversion() {
    // Test: 100 USD to BYN
    let (stdout, _) = run_command(&["--no-update", "--rate", "BYN:3.41", "100 usd to byn"]);
...
fn test_dollar_symbol_to_byn() {
    ...
    let (stdout, _) = run_command(&["--no-update", "--rate", "BYN:3.41", "100$ to byn"]);
    assert!(stdout.contains("byn"));
    // 100 * 3.41 = 341 BYN (no decimals for round numbers)
    let re = Regex::new(r"341 byn").unwrap();
    assert!(re.is_match(stdout.trim()));
...
fn test_unit_conversion() {
    let (stdout, _) = run_command(&["100 cm in m"]);
...
#[test]
fn test_version() {
    let (stdout, _) = run_command(&["--version"]);
...
fn test_variable_assignment() {
    let (stdout, _) = run_command(&["x = 5 + 3"]);
    assert!(stdout.contains("8.00") || stdout.contains("8"));
}

#[test]
fn test_error_handling() {
    let (_, stderr) = run_command(&["invalid expression"]);
    assert!(stderr.contains("Error"));
}

#[test]
fn test_complex_expression() {
    let (stdout, _) = run_command(&["2 * (3 + 4) / 2"]);
    assert!(stdout.contains("7"));
}

#[test]
fn test_scales() {
    let (stdout, _) = run_command(&["2k"]);
    assert!(stdout.contains("2.0k"));
}

#[test]
fn test_prettify_large_numbers() {
    let (stdout, _) = run_command(&["1000"]);
    assert!(stdout.contains("1.0k"));
}

#[test]
fn test_prettify_millions() {
    let (stdout, _) = run_command(&["1500000"]);
    assert!(stdout.contains("1.5M"));
}

#[test]
fn test_large_numbers() {
    let (stdout, _) = run_command(&["1000000000"]);
    assert!(stdout.contains("1.0B"));
}

#[test]
fn test_negative_numbers() {
    let (stdout, _) = run_command(&["-10 + 5"]);
    assert!(stdout.contains("-5"));
}

#[test]
fn test_unit_conversions() {
    let (stdout, _) = run_command(&["1000 m in km"]);
...
fn test_currency_conversions() {
    ...
    let (stdout, _) = run_command(&["--no-update", "--rate", "EUR:0.85", "100 usd in eur"]);
    assert!(stdout.contains("eur"));
    // 100 * 0.85 = 85.00 eur
    let re = Regex::new(r"85\.00 eur").unwrap();
    assert!(re.is_match(stdout.trim()));
...
fn test_expression_with_units_conversion() {
    let (stdout, _) = run_command(&["10 + 5 m in cm"]);
...
fn test_invalid_expression() {
    let (_, stderr) = run_command(&["invalid"]);
    assert!(stderr.contains("Error"));
}
...
fn test_missing_operand() {
    let (_, stderr) = run_command(&["2 +"]);
    assert!(stderr.contains("Error"));
}

#[test]
fn test_unsupported_unit() {
    let (_, stderr) = run_command(&["10 xyz in abc"]);
    assert!(stderr.contains("Error"));
}

#[test]
fn test_unsupported_currency() {
    let (_, stderr) = run_command(&["10 xyz in eur"]);
    assert!(stderr.contains("Error"));
}

#[test]
fn test_functions() {
    let (stdout, _) = run_command(&["sin(0)"]);
    assert!(stdout.contains("0"));
}

#[test]
fn test_scales_with_units() {
    let (stdout, _) = run_command(&["1k usd in jpy"]);
    assert!(stdout.contains("jpy"));
}

#[test]
fn test_multiple_operations() {
    let (stdout, _) = run_command(&["10 + 20 * 3 - 5"]);
    assert!(stdout.contains("65"));
}

#[test]
fn test_parentheses() {
    let (stdout, _) = run_command(&["(10 + 20) * 3"]);
    assert!(stdout.contains("90"));
}

#[test]
fn test_floating_point() {
    let (stdout, _) = run_command(&["10.5 + 2.3"]);
...
#[test]
fn test_modulo() {
    let (stdout, _) = run_command(&["10 % 3"]);
    assert!(stdout.contains("1"));
}

#[test]
fn test_history_commands() {
    let (stdout, _) = run_command(&["sum"]);
...
fn test_complex_currency_conversion() {
    ...
    let (stdout, _) = run_command(&["--no-update", "--rate", "JPY:154", "10 + 200000 usd in jpy"]);
    assert!(stdout.contains("jpy"));
...
fn test_complex_length_conversion() {
    let (stdout, _) = run_command(&["10.2 m in km"]);
...
fn test_large_number_with_unit_conversion() {
    // Use fixed rate: 1 USD = 0.85 EUR
    let (stdout, _) = run_command(&["--no-update", "--rate", "EUR:0.85", "1M usd in eur"]);
    assert!(stdout.contains("eur"));
...
fn test_mixed_operations_with_conversion() {
    // Use fixed rate: 1 USD = 154 JPY
    let (stdout, _) = run_command(&["--no-update", "--rate", "JPY:154", "100 usd * 2 in jpy"]);
    assert!(stdout.contains("jpy"));
...
fn test_comments() {
    ...
    let (stdout, _) = run_command(&["10 + 5 // this is a comment"]);
    ...
    let (stdout, _) = run_command(&["10% of 200 // calculate percentage"]);
    ...
    let (stdout, _) = run_command(&["50% of 100 USD // half of 100 dollars"]);
    ...
    let (_, stderr) = run_command(&["// just a comment"]);
    assert!(stderr.contains("Error"));
    ...
    let (stdout, _) = run_command(&["10 + 5 // comment # ignored"]);
    assert!(stdout.contains("15"));
}
...
fn test_percentage_expressions() {
    // Test "X% of Y" format
    let (stdout, _) = run_command(&["10% of 100"]);
    assert!(stdout.contains("10"));

    let (stdout, _) = run_command(&["50% of 200"]);
    assert!(stdout.contains("100"));

    // Test with units
    let (stdout, _) = run_command(&["25% of 100 USD"]);
    assert!(stdout.contains("25.00 USD"));

    // Test percentage operations
    let (stdout, _) = run_command(&["100 + 10%"]);
    assert!(stdout.contains("110"));

    let (stdout, _) = run_command(&["200 - 25%"]);
    assert!(stdout.contains("150"));

    let (stdout, _) = run_command(&["100 * 50%"]);
    assert!(stdout.contains("50"));

    let (stdout, _) = run_command(&["100 / 20%"]);
...
fn test_percentage_nested_parentheses() {
    ...
    let (stdout, _) = run_command(&["(50% of (100 + 100))"]);
    assert!(stdout.contains("100"), "got {}", stdout);

    // ((100 + 50%) + 10%) = (150 + 10%) = 165
    let (stdout, _) = run_command(&["((100 + 50%) + 10%)"]);
...
fn test_percentage_standalone_in_parens() {
    ...
    let (stdout, _) = run_command(&["(30% of 100)"]);
...
fn test_percentage_multiple_operations() {
    ...
    let (stdout, _) = run_command(&["50% of 200 + 50"]);
    assert!(stdout.contains("125"), "got {}", stdout);
    ...
    // 100 * 25% = 25
    let (stdout, _) = run_command(&["100 * 25%"]);
...
fn test_percentage_decimal_values() {
    // 12.5% of 80 = 10
    let (stdout, _) = run_command(&["12.5% of 80"]);
    assert!(stdout.contains("10"), "got {}", stdout);

    // 33.33% of 300 = 99.99
    let (stdout, _) = run_command(&["33.33% of 300"]);
    assert!(
    ...
    // (2.5% of 1000) = 25
    let (stdout, _) = run_command(&["(2.5% of 1000)"]);
    assert!(stdout.contains("25"), "got {}", stdout);
}
...
fn test_percentage_parens_with_arithmetic() {
    ...
    let (stdout, _) = run_command(&["2 * (50% of 100)"]);
    assert!(stdout.contains("100"), "got {}", stdout);

    // (50% of 100) * 2 = 50 * 2 = 100
    let (stdout, _) = run_command(&["(50% of 100) * 2"]);
    assert!(stdout.contains("100"), "got {}", stdout);

    // (50% of 100) + (25% of 200) = 50 + 50 = 100
    let (stdout, _) = run_command(&["(50% of 100) + (25% of 200)"]);
    assert!(stdout.contains("100"), "got {}", stdout);

    // (100 + 50%) / 3 = 150 / 3 = 50
    let (stdout, _) = run_command(&["(100 + 50%) / 3"]);
    assert!(stdout.contains("50"), "got {}", stdout);

    // 1000 - (10% of 1000) - (5% of 1000) = 1000 - 100 - 50 = 850
    let (stdout, _) = run_command(&["1000 - (10% of 1000) - (5% of 1000)"]);
...
fn test_percentage_parens_complex_nesting() {
    ...
    let (stdout, _) = run_command(&["((200 - 50%) + 20%)"]);
    assert!(stdout.contains("120"), "got {}", stdout);

    // (10% of (50% of 1000)) = 10% of 500 = 50
    let (stdout, _) = run_command(&["(10% of (50% of 1000))"]);
    assert!(stdout.contains("50"), "got {}", stdout);

    // ((100 + 100%) - 50%) = (200 - 50%) = 100
    let (stdout, _) = run_command(&["((100 + 100%) - 50%)"]);
    assert!(stdout.contains("100"), "got {}", stdout);
}
...
fn test_percentage_edge_cases() {
    // 0% of 100 = 0
    let (stdout, _) = run_command(&["0% of 100"]);
    assert!(stdout.contains("0"), "got {}", stdout);

    // 100% of 50 = 50
    let (stdout, _) = run_command(&["100% of 50"]);
    assert!(stdout.contains("50"), "got {}", stdout);

    // 200% of 50 = 100
    let (stdout, _) = run_command(&["200% of 50"]);
    assert!(stdout.contains("100"), "got {}", stdout);

    // (0% of 1000) = 0
    let (stdout, _) = run_command(&["(0% of 1000)"]);
    assert!(stdout.contains("0"), "got {}", stdout);

    // 100 + 0% = 100
    let (stdout, _) = run_command(&["100 + 0%"]);
    assert!(stdout.contains("100"), "got {}", stdout);

    // 100 - 100% = 0
    let (stdout, _) = run_command(&["100 - 100%"]);
    assert!(stdout.contains("0"), "got {}", stdout);
}
...
fn test_percentage_parens_real_world() {
    ...
    let (stdout, _) = run_command(&["100 + (50% of 100) - (20% of 150)"]);
    assert!(stdout.contains("120"), "got {}", stdout);
}
...
fn test_percentage_parens_division() {
    ...
    let (stdout, _) = run_command(&["1000 / (50% of 100)"]);
...
fn test_percentage_parens_multiplication() {
    ...
    let (stdout, _) = run_command(&["10 * (20% of 100)"]);
    assert!(stdout.contains("200"), "got {}", stdout);

    // (50% of 100) * (50% of 100) = 50 * 50 = 2500
    let (stdout, _) = run_command(&["(50% of 100) * (50% of 100)"]);
    assert!(
...
fn test_percentage_large_numbers() {
    ...
    let (stdout, _) = run_command(&["15% of 1000000"]);
    assert!(
    ...
    // (5% of 1000000) = 50000
    let (stdout, _) = run_command(&["(5% of 1000000)"]);
    assert!(
    ...
    // 1000000 + 10% = 1100000
    let (stdout, _) = run_command(&["1000000 + 10%"]);
    assert!(
...
fn test_percentage_small_numbers() {
    // 50% of 0.5 = 0.25
    let (stdout, _) = run_command(&["50% of 0.5"]);
    assert!(stdout.contains("0.25"), "got {}", stdout);

    // 10% of 0.01 = 0.001
    let (stdout, _) = run_command(&["10% of 0.01"]);
    assert!(
    ...
    // (25% of 0.8) = 0.2
    let (stdout, _) = run_command(&["(25% of 0.8)"]);
...
fn test_percentage_triple_nesting() {
    ...
    let (stdout, _) = run_command(&["(((100 + 50%) - 25%) + 10%)"]);
    assert!(
    ...
    let (stdout, _) = run_command(&["(10% of (20% of (50% of 1000)))"]);
    assert!(stdout.contains("10"), "got {}", stdout);

    // ((50% of 200) + (25% of (50% of 400))) = 100 + (25% of 200) = 100 + 50 = 150
    let (stdout, _) = run_command(&["((50% of 200) + (25% of (50% of 400)))"]);
    assert!(stdout.contains("150"), "got {}", stdout);
}
...
fn test_percentage_with_scales() {
    // 10% of 1k = 100
    let (stdout, _) = run_command(&["10% of 1k"]);
    assert!(stdout.contains("100"), "got {}", stdout);

    // (5% of 2M) = 100000 = 100k
    let (stdout, _) = run_command(&["(5% of 2M)"]);
    assert!(
    ...
    // 1k + 10% = 1100
    let (stdout, _) = run_command(&["1k + 10%"]);
    assert!(
    ...
    // (1M - 15%) = 850000 = 850k
    let (stdout, _) = run_command(&["(1M - 15%)"]);
    assert!(
...
fn test_percentage_shopping_scenarios() {
    ...
    let (stdout, _) = run_command(&["49.99 + (8.25% of 49.99)"]);
    ...
    let (stdout, _) = run_command(&["100 - 10 - (5% of 90)"]);
...
fn test_percentage_finance_scenarios() {
    ...
    // (10000 + 7%) = 10700
    let (stdout, _) = run_command(&["(10000 + 7%)"]);
    assert!(
    ...
    let (stdout, _) = run_command(&["5000 + (12% of 5000)"]);
    assert!(
    ...
    // (20% of 250000) = 50000
    let (stdout, _) = run_command(&["(20% of 250000)"]);
    assert!(
        stdout.contains("50") || stdout.contains("50.0k"),
        "got {}",
        stdout
    ...
    // (3% of 500000) = 15000
    let (stdout, _) = run_command(&["(3% of 500000)"]);
    assert!(
    ...
    let (stdout, _) = run_command(&["75% of 10000"]);
    assert!(
    ...
    let (stdout, _) = run_command(&["(22% of 30000)"]);
    assert!(
...
fn test_percentage_tip_scenarios() {
    ...
    // (18% of 85.50) = 15.39
    let (stdout, _) = run_command(&["(18% of 85.50)"]);
    assert!(
    ...
    let (stdout, _) = run_command(&["85.50 + (20% of 85.50)"]);
    assert!(
...
fn test_percentage_salary_scenarios() {
    ...
    let (stdout, _) = run_command(&["(75000 + 3%)"]);
    assert!(
    ...
    // (10% of 60000) = 6000
    let (stdout, _) = run_command(&["(10% of 60000)"]);
    assert!(
    ...
    // (80000 - 25%) = 60000
    let (stdout, _) = run_command(&["(80000 - 25%)"]);
    assert!(
    ...
    // (6% of 5000) = 300
    let (stdout, _) = run_command(&["(6% of 5000)"]);
...
fn test_percentage_fitness_scenarios() {
    ...
    // (2000 - 20%) = 1600
    let (stdout, _) = run_command(&["(2000 - 20%)"]);
    assert!(
    ...
    let (stdout, _) = run_command(&["(30% of 2500) / 4"]);
    assert!(
    ...
    // (15% of 200) = 30
    let (stdout, _) = run_command(&["(15% of 200)"]);
    assert!(stdout.contains("30"), "got {}", stdout);

    // Heart rate zones: 70% of max HR 190
    // (70% of 190) = 133
    let (stdout, _) = run_command(&["(70% of 190)"]);
...
fn test_percentage_business_scenarios() {
    ...
    let (stdout, _) = run_command(&["(30% of 50000)"]);
    assert!(
        stdout.contains("15") || stdout.contains("15.0k"),
        "got {}",
        stdout
    ...
    // (40 + 60%) = 64
    let (stdout, _) = run_command(&["(40 + 60%)"]);
    assert!(stdout.contains("64"), "got {}", stdout);

    // Inventory shrinkage: 2% of $100000 inventory
    // (2% of 100000) = 2000
    let (stdout, _) = run_command(&["(2% of 100000)"]);
    assert!(
    ...
    let (stdout, _) = run_command(&["10000 - (5% of 10000)"]);
    assert!(
    ...
    let (stdout, _) = run_command(&["(1000000 + 12%)"]);
    assert!(
...
fn test_percentage_education_scenarios() {
    ...
    let (stdout, _) = run_command(&["40000 - (75% of 40000)"]);
    assert!(
...
fn test_percentage_with_negative_base() {
    ...
    // -50 - 20% = -50 - (-10) = -40
    let (stdout, _) = run_command(&["-50 - 20%"]);
    assert!(stdout.contains("-40"), "got {}", stdout);
}
...
fn test_percentage_order_of_operations() {
    ...
    let (stdout, _) = run_command(&["150 - 20%"]);
    assert!(stdout.contains("120"), "got {}", stdout);
    ...
    let (stdout, _) = run_command(&["20% of 150"]);
    assert!(stdout.contains("30"), "got {}", stdout);
}
...
fn test_investment_calculator() {
    // 10000 * (1 + 0.07)^10
    let (stdout, _) = run_command(&["10000 * (1 + 0.07)^10"]);
...
fn test_compound_interest() {
    ...
    let (stdout, _) = run_command(&["5000 * (1 + 0.08/12)^(12*30)"]);
...
fn test_projectile_motion() {
    // 9.8 * 5^2 / 2
    let (stdout, _) = run_command(&["9.8 * 5^2 / 2"]);
    // Result: 122.5
...
fn test_circle_area() {
    // π * 15^2
    let (stdout, _) = run_command(&["π * 15^2"]);
    // Result: 706.86 ≈ 707
...
fn test_sphere_volume() {
    // (4/3) * π * 8^3
    let (stdout, _) = run_command(&["(4/3) * π * 8^3"]);
    // Result: 2144.66 ≈ 2.1k
...
fn test_speed_of_light_conversion() {
    // 299792458 / 1609.34
    let (stdout, _) = run_command(&["299792458 / 1609.34"]);
...
fn test_salary_monthly() {
    // 125000 / 12
    let (stdout, _) = run_command(&["125000 / 12"]);
...
fn test_hourly_rate() {
    // 10416.67 / 160
    let (stdout, _) = run_command(&["10416.67 / 160"]);
    // Result: 65.10
...
fn test_bitcoin_to_usd() {
    // 0.5 * 43000
    let (stdout, _) = run_command(&["0.5 * 43000"]);
    // Result: 21500 ≈ 21.5k
...
fn test_gb_to_mb() {
    // 256 * 1024
    let (stdout, _) = run_command(&["256 * 1024"]);
...
fn test_revenue_per_day() {
    // 1500000 / 365
    let (stdout, _) = run_command(&["1500000 / 365"]);
    // Result: 4109.59 ≈ 4.1k
...
fn test_profit_margin() {
    ...
    let (stdout, _) = run_command(&["(89.99 - 45.50) / 89.99 * 100"]);
    // Result: 49.44
...
fn test_tax_calculation() {
    // 75000 * 0.22
    let (stdout, _) = run_command(&["75000 * 0.22"]);
    // Result: 16500 ≈ 16.5k
...
fn test_tip_calculator() {
    // 145.50 * 0.20
    let (stdout, _) = run_command(&["145.50 * 0.20"]);
    // Result: 29.10
...
fn test_sale_discount() {
    // 299.99 * (1 - 0.30)
    let (stdout, _) = run_command(&["299.99 * (1 - 0.30)"]);
    // Result: 209.99 ≈ 210
...
fn test_growth_rate() {
    ...
    let (stdout, _) = run_command(&["(15000 - 12000) / 12000 * 100"]);
    // Result: 25.00
...
fn test_trigonometric_sine() {
    // sin(π/6) = 0.5
    let (stdout, _) = run_command(&["sin(π/6)"]);
...
fn test_trigonometric_cosine() {
    // cos(π/3) = 0.5
    let (stdout, _) = run_command(&["cos(π/3)"]);
    assert!(stdout.contains("0.5") || stdout.contains("0.49"));
}
...
fn test_trigonometric_tangent() {
    // tan(π/4) = 1
    let (stdout, _) = run_command(&["tan(π/4)"]);
...
fn test_natural_logarithm() {
    // ln(e) = 1
    let (stdout, _) = run_command(&["ln(e)"]);
    assert!(stdout.contains("1"));

    // ln(2.71828) ≈ 1
    let (stdout, _) = run_command(&["ln(2.71828)"]);
    assert!(stdout.contains("1"));

    // ln(10) ≈ 2.303
    let (stdout, _) = run_command(&["ln(10)"]);
...
fn test_logarithm_base_10() {
    // log(100) = 2
    let (stdout, _) = run_command(&["log(100)"]);
...
fn test_square_root() {
    // sqrt(144) = 12
    let (stdout, _) = run_command(&["sqrt(144)"]);
    assert!(stdout.contains("12"));

    // sqrt(2) ≈ 1.414
    let (stdout, _) = run_command(&["sqrt(2)"]);
    assert!(stdout.contains("1.4"));

    // Nested: sqrt(sqrt(256)) = sqrt(16) = 4
    let (stdout, _) = run_command(&["sqrt(sqrt(256))"]);
...
fn test_absolute_value() {
    // abs(-42)
    let (stdout, _) = run_command(&["abs(-42)"]);
...
fn test_rounding_functions() {
    // ceil(3.2) = 4
    let (stdout, _) = run_command(&["ceil(3.2)"]);
    assert!(stdout.contains("4"));

    // floor(3.8) = 3
    let (stdout, _) = run_command(&["floor(3.8)"]);
    assert!(stdout.contains("3"));

    // round(3.5) = 4
    let (stdout, _) = run_command(&["round(3.5)"]);
...
fn test_hyperbolic_functions() {
    // sinh(1)
    let (stdout, _) = run_command(&["sinh(1)"]);
    assert!(stdout.contains("1.17") || stdout.contains("1.18"));

    // cosh(0) = 1
    let (stdout, _) = run_command(&["cosh(0)"]);
    assert!(stdout.contains("1"));
}
...
fn test_kinetic_energy() {
    ...
    let (stdout, _) = run_command(&["0.5 * 10 * 20^2"]);
...
fn test_gravitational_potential_energy() {
    ...
    // PE = 5 * 9.8 * 10 = 490J
    let (stdout, _) = run_command(&["5 * 9.8 * 10"]);
...
fn test_pythagorean_theorem() {
    ...
    let (stdout, _) = run_command(&["sqrt(3^2 + 4^2)"]);
    assert!(stdout.contains("5"));
}
...
fn test_circle_circumference() {
    // C = 2 * π * r, where r=7
    // C = 2 * π * 7 ≈ 43.98
    let (stdout, _) = run_command(&["2 * π * 7"]);
...
fn test_cylinder_volume() {
    ...
    // V = π * 25 * 10 ≈ 785.4
    let (stdout, _) = run_command(&["π * 5^2 * 10"]);
...
fn test_standard_deviation_components() {
    ...
    let (stdout, _) = run_command(&["((2-4)^2 + (4-4)^2 + (6-4)^2) / 3"]);
...
fn test_z_score() {
    ...
    // z = (85 - 70) / 10 = 1.5
    let (stdout, _) = run_command(&["(85 - 70) / 10"]);
...
fn test_simple_interest() {
    ...
    // I = 1000 * 0.05 * 3 = 150
    let (stdout, _) = run_command(&["1000 * 0.05 * 3"]);
    assert!(stdout.contains("150"));
}
...
fn test_future_value() {
    ...
    let (stdout, _) = run_command(&["5000 * (1.06)^5"]);
...
fn test_present_value() {
    ...
    let (stdout, _) = run_command(&["10000 / (1.08)^10"]);
...
fn test_compound_annual_growth_rate() {
    ...
    let (stdout, _) = run_command(&["(15000/10000)^(1/5) - 1"]);
...
fn test_binary_operations() {
    ...
    let (stdout, _) = run_command(&["1024^3"]);
...
fn test_base_conversion_calculation() {
    ...
    let (stdout, _) = run_command(&["1*2^3 + 1*2^2 + 1*2^1 + 1*2^0"]);
    assert!(stdout.contains("15"));
}
...
fn test_deeply_nested_expression() {
    ...
    let (stdout, _) = run_command(&["((2 + 3) * 4 - (6 / 2))^2"]);
...
fn test_scientific_notation_operations() {
    ...
    let (stdout, _) = run_command(&["3e8 * 60"]);
...
fn test_underscore_in_numbers() {
    ...
    let (stdout, _) = run_command(&["1_000_000"]);
    assert!(stdout.contains("1.0M") || stdout.contains("1M"));

    // Multiple underscores
    let (stdout, _) = run_command(&["1_000_000 + 500_000"]);
    assert!(stdout.contains("1.5M"));
}
...
fn test_no_space_units() {
    ...
    let (stdout, _) = run_command(&["100USD"]);
    assert!(stdout.contains("100") && stdout.contains("USD"));

    // 1_000_000USD
    let (stdout, _) = run_command(&["1_000_000USD"]);
...
fn test_inverse_hyperbolic_functions() {
    // asinh(1) ≈ 0.881
    let (stdout, _) = run_command(&["asinh(1)"]);
    assert!(stdout.contains("0.8") || stdout.contains("0.9"));

    // acosh(2) ≈ 1.317
    let (stdout, _) = run_command(&["acosh(2)"]);
    assert!(stdout.contains("1.3") || stdout.contains("1.4"));

    // atanh(0.5) ≈ 0.549
    let (stdout, _) = run_command(&["atanh(0.5)"]);
...
fn test_min_max_functions() {
    // min(5, 3, 8) = 3
    let (stdout, _) = run_command(&["min(5, 3, 8)"]);
    assert!(stdout.contains("3"));

    // max(5, 3, 8) = 8
    let (stdout, _) = run_command(&["max(5, 3, 8)"]);
    assert!(stdout.contains("8"));

    // Nested: max(min(10, 5), min(8, 3)) = max(5, 3) = 5
    let (stdout, _) = run_command(&["max(min(10, 5), min(8, 3))"]);
    assert!(stdout.contains("5"));
}
...
fn test_temperature_conversions() {
    ...
    let (stdout, _) = run_command(&["0 celsius to fahrenheit"]);
    assert!(stdout.contains("32"));

    // 100 celsius to fahrenheit = 212
    let (stdout, _) = run_command(&["100 celsius to fahrenheit"]);
    assert!(stdout.contains("212"));

    // 32 fahrenheit to celsius = 0
    let (stdout, _) = run_command(&["32 fahrenheit to celsius"]);
    assert!(stdout.contains("0"));

    // 273.15 kelvin to celsius = 0
    let (stdout, _) = run_command(&["273.15 kelvin to celsius"]);
...
fn test_time_conversions() {
    ...
    let (stdout, _) = run_command(&["60 seconds to minutes"]);
    assert!(stdout.contains("1"));

    // 24 hours to days = 1
    let (stdout, _) = run_command(&["24 hours to days"]);
    assert!(stdout.contains("1"));

    // 365 days to years = 1
    let (stdout, _) = run_command(&["365 days to years"]);
    assert!(stdout.contains("1"));
}
...
fn test_area_conversions() {
    ...
    let (stdout, _) = run_command(&["10000 m2 to hectare"]);
    assert!(stdout.contains("1"));

    // 4046.86 m2 to acre ≈ 1
    let (stdout, _) = run_command(&["4046.86 m2 to acre"]);
    assert!(stdout.contains("1"));
}
...
fn test_volume_conversions() {
    // 1000 liters to m3 = 1
    let (stdout, _) = run_command(&["1000 liters to m3"]);
    assert!(stdout.contains("1"));

    // 4 quarts to gallon ≈ 1
    let (stdout, _) = run_command(&["4 quarts to gallon"]);
    assert!(stdout.contains("1"));
}
...
fn test_weight_conversions() {
    // 1000 grams to kg
    let (stdout, _) = run_command(&["1000000 grams to tonne"]);
    assert!(stdout.contains("1"));

    // 453.592 grams to pound = 1
    let (stdout, _) = run_command(&["453.592 grams to pound"]);
    assert!(stdout.contains("1"));
...
fn test_angular_conversions() {
    ...
    let (stdout, _) = run_command(&["π radians to degrees"]);
    assert!(stdout.contains("180"));

    // 90 degrees to radians ≈ 1.571 (π/2)
    let (stdout, _) = run_command(&["90 degrees to radians"]);
...
fn test_data_conversions() {
    // 8 bits to bytes = 1
    let (stdout, _) = run_command(&["8 bits to byte"]);
    assert!(stdout.contains("1"));

    // 1024 bytes to... (check if KB is supported)
    let (stdout, _) = run_command(&["8192 bits to bytes"]);
...
fn test_speed_conversions() {
    // 1 m/s to km/h ≈ 3.6
    let (stdout, _) = run_command(&["1 m/s to km/h"]);
    assert!(stdout.contains("3.6") || stdout.contains("3.5"));

    // 60 mph to km/h ≈ 96.56
    let (stdout, _) = run_command(&["60 mph to km/h"]);
...
fn test_sign_function() {
    // sign(42) = 1
    let (stdout, _) = run_command(&["sign(42)"]);
    assert!(stdout.contains("1"));
    ...
    // sign(0) = 0
    let (stdout, _) = run_command(&["sign(0)"]);
    assert!(stdout.contains("0"));
}
...
fn test_int_function() {
    // int(3.7) = 3
    let (stdout, _) = run_command(&["int(3.7)"]);
    assert!(stdout.contains("3"));

    // int(-2.9) = -2
    let (stdout, _) = run_command(&["int(-2.9)"]);
    assert!(stdout.contains("-2"));
}
...
fn test_cooking_recipe_conversion() {
    ...
    let (stdout, _) = run_command(&["2 cup to liters"]);
...
fn test_travel_distance_calculation() {
    ...
    let (stdout, _) = run_command(&["500 miles to km"]);
...
fn test_bmi_calculation() {
    ...
    let (stdout, _) = run_command(&["70 / (1.75^2)"]);
...
fn test_fuel_efficiency() {
    ...
    // 30 mpg = ? L/100km
    // Formula: 235.214 / mpg
    let (stdout, _) = run_command(&["235.214 / 30"]);
...
fn test_currency_word_dollars_to_euros() {
    let (stdout, _) = run_command(&["--no-update", "--rate", "EUR:0.85", "100 dollars to euros"]);
...
fn test_currency_word_yuan_rmb() {
    // Test yuan mapping
    let (stdout, _) = run_command(&["--no-update", "--rate", "CNY:7.2", "100 yuan to usd"]);
    let stdout_lower = stdout.to_lowercase();
    assert!(stdout_lower.contains("usd"));

    // Test rmb mapping (alternative for CNY)
    let (stdout, _) = run_command(&["--no-update", "--rate", "CNY:7.2", "100 rmb to usd"]);
    let stdout_lower = stdout.to_lowercase();
    assert!(stdout_lower.contains("usd"));
}
...
fn test_currency_word_mixed_case() {
    let (stdout, _) = run_command(&["--no-update", "--rate", "EUR:0.85", "50 DOLLARS into EUROS"]);
    let stdout_lower = stdout.to_lowercase();
    assert!(stdout_lower.contains("eur"));
}
...
fn test_currency_word_singular_plural() {
    ...
    let (stdout, _) = run_command(&["--no-update", "--rate", "EUR:0.85", "1 dollar to euro"]);
    let stdout_lower = stdout.to_lowercase();
    assert!(stdout_lower.contains("eur"));

    // Test plural "dollars"
    let (stdout, _) = run_command(&["--no-update", "--rate", "EUR:0.85", "10 dollars to euros"]);
    let stdout_lower = stdout.to_lowercase();
    assert!(stdout_lower.contains("eur"));
}
