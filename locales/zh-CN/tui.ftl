...
tui-help-ctrlshift-n-desc = 数字格式选择器
...
# 数字格式选择
tui-number-format-title = 数字格式
tui-number-decimals-title = 最大小数位
tui-number-format-footer = ↑/↓ 选择   ←/→ 切换列表   Enter 应用   Esc 关闭
...
tui-number-format-set-status = 数字格式已设置：{$format}，最大小数位：{$decimals}
