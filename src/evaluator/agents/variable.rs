...
impl Agent for VariableAgent {
    ...
    fn process(&self, input: &str, state: &mut AppState, _config: &crate::config::Config) -> Option<(String, bool)> {
        ...
        if parts.len() == 2 {
            ...
            let mut vars_clone = state.variables.read().unwrap().clone();
            if let Some(val_str) = evaluate_expr(val_expr, &mut vars_clone, &state.history.read().unwrap().clone(), &state.length_units, &state.time_units, &state.temperature_units, &state.area_units, &state.volume_units, &state.weight_units, &state.angular_units, &state.data_units, &state.speed_units, &state.rates, &std::collections::HashMap::new()) {
                ...
                if let Some(num_str) = parts_val.first() {
                    if let Ok(val) = num_str.parse::<f64>() {
                        ...
                        state.variables.write().unwrap().insert(var.to_string(), (val, unit));
                        // Invalidate caches since variables changed
                        state.display_cache.write().unwrap().clear();
                        state.highlight_cache.write().unwrap().clear();
                    }
                }
