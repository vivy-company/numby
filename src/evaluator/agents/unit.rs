...
impl Agent for UnitAgent {
    ...
    fn process(&self, input: &str, state: &mut AppState, config: &crate::config::Config) -> Option<(String, bool)> {
        ...
        if let Some((kw, pos)) = conversion_keyword {
            ...
            if let Some(val) = evaluate_unit_conversion(left, right, &state.variables.read().unwrap().clone(), &state.history.read().unwrap().clone(), &state.length_units, &state.time_units, &state.temperature_units, &state.area_units, &state.volume_units, &state.weight_units, &state.angular_units, &state.data_units, &state.speed_units, &state.rates, &config.custom_units) {
            ...
            if let Some(left_result) = evaluate_expr(
                ...
                if let Some(val) = evaluate_unit_conversion(&left_result, right, &vars_clone, &state.history.read().unwrap().clone(), &config.length_units, &config.time_units, &config.temperature_units, &config.area_units, &config.volume_units, &config.weight_units, &config.angular_units, &config.data_units, &config.speed_units, &config.currencies, &config.custom_units) {
                    return Some((val, true));
                }
            }
        }
